...
threevector DebyeHuckelField(threevector Position, double Radius){
	double Charge=0.0;//2000*echarge;
...
*/
	...
	threevector Position(dist(mt),dist(mt),10.0);	// Distance normalised to dust grain size, Start 10 Radii above dust
...
//	std::cout << "\n" << Position << "\t\t" << Velocity;
	...
	// std::cout << "\n" << TimeStep << "\t" << echarge/Me << "\n";
	while(Position.mag3() > 1.0 && Position.getz() > 0.0){
		OldPosition = Position;
		Position+=TimeStep*Velocity;
		MeanPosition = 0.5*(Position+OldPosition);
		MeanVelocity = ((Position-OldPosition)*(1/TimeStep));
//		DeltaV=TimeStep*(echarge/Me)*(DebyeHuckelField(MeanPosition,Radius)*(1/pow(Radius,3))+BMag*Velocity^Bhat); // FOR ELECTRONS
